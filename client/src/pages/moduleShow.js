...
export class moduleShow extends Component {
  ...
  render() {
    ...
    var chapter = this.props.lessons.lesson.chapters.find(
      (chap) => chap.chapId === this.props.match.params.chapId
    );
    console.log(chapter);
    var quiz = chapter.quiz.find(
      (qu) => qu.ref === this.props.match.params.ref
    );
    var reading = chapter.reading.find(
      (re) => re.ref === this.props.match.params.ref
    );
    var videos = chapter.videos.find(
      (vi) => vi.ref === this.props.match.params.ref
    );
    console.log(quiz, reading, videos, this.props.match.params.ref);
    if (quiz || reading || videos) {
      if (quiz && quiz.complited.find((i) => this.props.user.id === i)) {
        iscomplited = true;
        console.log(this.props.user.id, iscomplited);
      }
      if (reading && reading.complited.find((i) => this.props.user.id === i)) {
        iscomplited = true;
        console.log(this.props.user.id, iscomplited);
      }
      if (videos && videos.complited.find((i) => this.props.user.id === i)) {
        iscomplited = true;
        console.log(this.props.user.id, iscomplited);
      }
    }
            ...
            <Grid item sm={12} md={4}>
              <div className={classes.infobox}>
                ...
                <div className={classes.lack}>
                  <Typography variant="body2">{module.data.time}</Typography>
