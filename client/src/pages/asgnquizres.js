...
export class quizres extends Component {
  componentDidMount() {
    ...
    this.props.getResult(this.props.match.params.ref, this.props.user.id);
  }
  render() {
    const {
      classes,
      result: { noques, nocurr, nosub, percent },
      ploading,
    } = this.props;
    ...
    return (
      <>
                  ...
                  <CardContent className={classes.contain}>
                    <div className={classes.upper}>
                      <Typography style={{ fontSize: "1.2rem" }}>
                        <strong>Practice Test Complited</strong>
                        <p style={{ color: "blue" }} className={classes.per}>
                          {percent.toString() + "%"}
                        </p>
                      </Typography>
                      <Typography>
                        Congratulations! You have scored{" "}
                        {percent.toString() + "%"} in this test
                      </Typography>
                    </div>
                    <div className={classes.middle}>
                      <Typography className={classes.text}>
                        No of Questions - {noques}
                      </Typography>
                      <Typography className={classes.text}>
                        Questions Attempted - {nosub}
                      </Typography>
                      <Typography className={classes.text}>
                        Answers Correct - {nocurr}
                      </Typography>
                      <br />
                      <Typography className={classes.total}>
                        Total Score : {nocurr} / {noques}
                      </Typography>
                    </div>
                  </CardContent>
        ...
        <p style={{ color: "red", textAlign: "center", fontSize: "0.8rem" }}>
          (Check result isn't implimented yet.)
        </p>
      </>
    );
  }
